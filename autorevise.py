from autorevise_util import joints_moving_together, find_segments, connected_joint_close, gradual_limits_on_peaks, find_all_peaks, find_next_valley, is_spike, find_occlusion
...
def autorevise(video_path):
    ...
    if action.label == Label.HAND_SHAKE or action.label == Label.FIST_BUMP:
        ...
        approach, action, leave = find_occlusion_based(vl.video.tracked_persons.values(), approach, leave)
...
def find_arm_valley_contact(persons, approach, action, leave):
    ...
    for person in persons:
        ...
        start, end, next_valley, prev_valley = find_occlusion(subsigsr, subsigsl, leave.end)
...
def find_contact(persons, approach, action, leave):
    ...
    res = []
    for wrist in wrists:
        l = joints_moving_together(wrist, length, approach.start, 1.0)
    ...
    con = res[index]
    start, end = connected_joint_close(res[index], wrists[index])
...
def action_to_leave(approach, action, leave, vl, gap):
    ...
    for tp in vl.video.tracked_persons.values():
        ...
        occ_s, occ_e, next_valley, prev_valley = find_occlusion(subsigsr, subsigsl, leave.end)
        if occ_s > 0:
    ...
    if person is not None:
        ...
        occ_start, occ_end, next_valley, prev_valley = find_occlusion(subsigsr, subsigsl, leave.end)
